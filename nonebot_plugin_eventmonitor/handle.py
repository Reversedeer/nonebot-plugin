...
class Eventmonitor:
    ...
    async def admin_chance(
        ...
        if not (await utils.check_admin(utils.g_temp, str(event.group_id))):
            return
        bot_qq = int(bot.self_id)
        rely_msg: str = await config.admin_changer(
            event.sub_type,
            event.user_id,
            bot_qq,
        )
        ...
        if "开启" in command or "开始" in command:
                ...
                if not (await utils.check_txt_to_img(utils.check_txt_img)):
                    ...
                    await matcher.send(
                        MessageSegment.image(
                            await txt_to_img.txt_to_img(f"{name}功能已开启喵")
                        )
                    )
            ...
            if key := utils.get_command_type(command):
                ...
                if not (await utils.check_txt_to_img(utils.check_txt_img)):
                    ...
                    await matcher.send(
                        MessageSegment.image(
                            await txt_to_img.txt_to_img(f"{name}功能已关闭喵")
                        )
                    )
    ...
    async def check_bot(matcher: Matcher) -> None:
        ...
        try:
            (
                code,
                error,
            ) = await eventmonitor.check_update(matcher)
            if error:
                logger.error(
                    f"错误: {error}",
                    "插件检查更新",
                ...
                await matcher.send(
                    f"下载更新插件nonebot-plugin-evenrmonitor时发生网络错误:\n{error}"
                )
        except Exception as e:
            logger.error(
                "备份更新插件nonebot-plugin-evenrmonitor时发生错误",
                e=e,
            )
            await matcher.send(
                f"备份更新插件nonebot-plugin-evenrmonitor时发生错误 \n{type(e)}: {e}"
            )
        else:
            if code == 200:
    ...
    async def check_update(matcher: Matcher):
        ...
        if data:
            ...
            if utils.current_version < latest_version:
                ...
                logger.info(
                    f"检测到插件nonebot-plugin-evenrmonitor有更新:\n"
                ...
                tar_gz_url = (await update.fetch_data(tar_gz_url)).headers.get(
                    "Location"
                )
                if await update.download_file(
                    tar_gz_url,
                    update.latest_tar_gz,
                ):
                ...
                logger.info(
                    f"自动获取版本成功：{latest_version}，当前版本为最新版，无需更新..."
                )
                await matcher.send(
                    f"自动获取版本成功：{latest_version}，当前版本为最新版，无需更新..."
                )
                ...
                logger.warning(
                    f"当前版本{utils.current_version}已大于最新版本，不要随意修改文件喵~"
                )
                await matcher.send(
                    f"当前版本{utils.current_version}已大于最新版本，不要随意修改文件喵~"
                )
    ...
    async def _file_handle():
        ...
        try:
            ...
            raise e
        os.system(
            f"poetry run pip install -r {(update_info_file / 'requirements.txt').absolute()}"
        )
