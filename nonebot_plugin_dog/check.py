...
async def check_update(bot: Bot):
    ...
    if data:
        ...
        if utils.current_version != latest_version:
            ...
            logger.info(
                f"检测插件dog:有更新，当前版本：{utils.current_version}，最新版本：{latest_version}"
            )
            ...
            if await download_file(tar_gz_url, config.latest_tar_gz):
                ...
                logger.warning(
                    f"下载最新版本失败..请检查网络是否通畅.版本号：{latest_version}"
                )
                await bot.send_private_msg(
                    user_id=int(list(bot.config.superusers)[0]),
...
async def _file_handle(latest_version: str) -> str:
    ...
    os.system(
        f"poetry run pip install -r {(update_info_file / 'pyproject.toml').absolute()}"
    )
