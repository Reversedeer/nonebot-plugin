...
class Config:
    release_url = (
        "https://api.github.com/repos/Reversedeer/nonebot_plugin_dog/releases/latest"
    )
    ...
    async def remind(bot: Bot):
        ...
        if system != "windows":
            restart = config.config_path / "restart.sh"
        ...
        for _ in range(3):
            try:
                async with httpx.AsyncClient() as client:
                    res = await client.get(config.release_url)
