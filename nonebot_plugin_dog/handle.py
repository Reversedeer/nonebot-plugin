...
class Dog:
    ...
    async def lickdog(matcher: Matcher, event: GroupMessageEvent):  # 定义异步函数 dog
        ...
        if (
            ...
            await matcher.finish(
                MessageSegment.text(
                    f"不要深情了喵，休息{utils.dog_cd - cd:.0f}秒后再找我喵~"
                ),
                at_sender=True,
                block=True,
            )
    ...
    async def laugh(event: GroupMessageEvent, matcher: Matcher):  # 定义异步函数 laugh
        ...
        if (
            ...
            await matcher.finish(
                MessageSegment.text(
                    f"我在准备更精彩的笑话喵，等待{utils.laugh_cd - cd:.0f}秒后再找我喵~"
                ),
                at_sender=True,
                block=True,
            )
    ...
    async def hitokoto(
        ...
        if (
            ...
            await matcher.finish(
                MessageSegment.text(
                    f"休息 {utils.hitokoto_cd - cd:.0f}秒后才能再使用喵~"
                ),
                at_sender=True,
                block=True,
            )
    ...
    async def wenan(event: GroupMessageEvent, matcher: Matcher):  # 定义异步函数wenan
        ...
        if (
            ...
            await matcher.finish(
                MessageSegment.text(
                    f"文案准备中喵，等待{utils.wenan_cd - cd:.0f}秒后再找我喵~"
                ),
                at_sender=True,
                block=True,
            )
        ...
        try:
            ...
            if error:
                ...
                await bot.send_private_msg(
                    ...
                    message=f"更新插件dog更新时发生未知错误 {error}",
                )
            ...
            if code == 200:
                await bot.send_private_msg(
                    user_id=event.user_id, message="更新完毕，请重启bot...."
                )
