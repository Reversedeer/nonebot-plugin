...
class Hyp:
    @staticmethod
    async def hyp(matcher: Matcher, arg: Message = CommandArg()) -> NoReturn:
        """hypixel"""
        ...
        online, rank, last_login, level = await asyncio.gather(*tasks)
        data = {
            "online": online,
            "rank": rank,
    ...
    @staticmethod
    async def mc(matcher: Matcher, arg: Message = CommandArg()):
        """minecraft"""
        args = str(arg).strip().split()
        if len(args) < 1:
            await matcher.finish("请提供一个有效的ID")
    ...
    @staticmethod
    async def bw(matcher: Matcher, arg: Message = CommandArg()):
        """bw"""
        args = str(arg).strip().split()
        if len(args) < 1:
            await matcher.finish("请提供一个有效的ID")
        ...
        players_data = await api.player_data(uid)
        data_a = players_data["player_data"]
        data = await api.get_players_bedwars(data_a)
        reply = await msg.send_bw_msg(data, uid)
        await matcher.finish(reply)
